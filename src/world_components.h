 ...
 * fill. The production map is 16384x2048, and every solid cell in it is one
...
/* The longest side of a query region, in cells. The workspace carries a visited
   ...
   384 so that the region can hold the largest body the terrain system can
   carry, a large slot's 384x384. The visited bitmap grows with the square of
   this and is 18 KiB at 384, which is nothing beside the component's own cell
   list. */
#define WORLD_COMPONENT_MAX_SPAN 384

/* The largest component the workspace can hold. A component that would exceed
   it stops the search and reports WORLD_COMPONENT_TOO_LARGE. 49152 cells is a
   220x220 block of rock — four times the building-sized slab this allowed
   before, at the player's request for much larger bodies. The two coordinate
   arrays are the cost: 384 KiB of workspace, held once by the detach system
   rather than on anyone's stack. */
#define WORLD_COMPONENT_MAX_CELLS 49152
