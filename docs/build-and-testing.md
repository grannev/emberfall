...
Собирает и запускает `tests/world_tests.c` — headless-набор проверок ядра
симуляции. Результат: `build/emberfall-tests`.
...
`WorldInit` выделяет только CPU-состояние, а `WorldInitRenderer` — единственная
функция мира, которой нужен GL-контекст, — не вызывается. Это разделение нужно
сохранять; из-за него набор гоняется за доли секунды и без display, аудио и
X-сервера.

Набор собирается из `tests/world_tests.c`, `src/world.c` и `src/player.c` и
покрывает инварианты, которые иначе держались бы только на памяти
разработчика:
...
- boost пробуривает rock, сопротивление бурения никогда не останавливает игрока,
  и игрок не заканчивает кадр внутри твёрдой породы.
