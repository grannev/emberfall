...
Единственные места, где жидкость знает о персонаже и о телах. Первый
выставляет персонажу квадратичное сопротивление среды (`Player.fluidDrag`),
расталкивает воду при нырке и выходе, приподнимает её под быстрым низким
пролётом и шлёт `GAME_EVENT_LIQUID_SPLASH`/`GAME_EVENT_LIQUID_RIPPLE`; второй
на фиксированном шаге даёт бодрствующим телам плавучесть по плотности,
сопротивление и всплеск. Оба пишут мир только через `WorldPushLiquid`.
Владеет обоими `GameState`.
...
- Мир 16384×2048 выводится через резидентный кэш страниц 256×256, а не через
