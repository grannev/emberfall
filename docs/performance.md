# Производительность и память

Этот документ хранит воспроизводимые измерения Emberfall. Wall-clock числа
информационные: сравнивать результаты нужно на одной машине и в одинаковом
режиме. Счётчики `processedCells`, `processedChunks`, active/sleeping chunks и
dirty regions не зависят от частоты CPU и подходят для regression-проверок.

## Команды

```sh
make bench
make bench BENCH_ARGS="--ticks 60"
make profile
make asan
make ubsan
```

`make bench` не открывает окно и не инициализирует GPU/audio. Он использует мир
реального размера 16384×1440, фиксированный seed `0x00e6be11` и по 180 ticks в
десяти сценариях: settled world, sand, water, fire/lava, explosion, mass
destruction, boost drilling, force, cryo и mixed chaos. Время каждого scenario
включает его gameplay action и `WorldUpdate`, но не повторную генерацию перед
сценарием.

GPU uploads и render preparation в headless таблице помечены `n/a`, потому что
benchmark намеренно не создаёт GL context. Runtime `WorldRendererStats`
показывает dirty regions, texture uploads, uploaded bytes, число видимых и
резидентных страниц, число привязок страниц за кадр и совокупное время CPU
preparation/upload — всё это выводится в debug HUD строкой `RENDER`.

## Baseline 2026-08-31

Commit до audit: `34bf7c3`. Машина: Intel Core i5-6300U (2 cores/4 threads),
Linux x86_64 7.1.4, GCC 16.1.1, raylib 6.0.0. Release: `-O2`.

- Чистая release-сборка: 4,39 s.
- Все 41 headless tests: 10,16 s с компиляцией.
- Xvfb/llvmpipe smoke-test: 5,49 s, код возврата 0.
- Временный pre-suite probe: generation 463 ms; 120 локально активированных
  ticks — 1,66 ms/tick; 23 active chunks в конце.
- Valgrind Massif peak полезного heap: 288 483 448 B (275,12 MiB).

Первый постоянный `make bench` после добавления только instrumentation:

| Scenario | avg ms | p50 | p95 | p99 | cells/tick | chunks/tick |
|---|---:|---:|---:|---:|---:|---:|
| settled world | 0.594 | 0.530 | 0.889 | 0.991 | 0 | 0 |
| falling sand | 2.042 | 1.801 | 2.863 | 6.619 | 59 630 | 58 |
| large water | 4.333 | 4.045 | 5.871 | 7.215 | 86 664 | 84 |
| fire and lava | 2.678 | 2.247 | 4.404 | 5.257 | 52 929 | 51 |
| large explosion | 1.727 | 1.711 | 2.595 | 6.962 | 44 691 | 43 |
| mass destruction | 0.998 | 0.888 | 1.358 | 2.205 | 23 045 | 22 |
| boost drilling | 1.050 | 0.954 | 1.528 | 2.251 | 27 904 | 27 |
| force ability | 2.138 | 1.876 | 3.190 | 5.191 | 48 372 | 47 |
| cryo ability | 3.966 | 3.719 | 5.671 | 6.278 | 87 648 | 85 |
| chaotic mixed | 5.864 | 5.535 | 7.871 | 9.815 | 139 628 | 136 |

Init: 57,2 ms; generate: 465,3 ms; regenerate: 488,7 ms. Текущий resident
на момент измерения был 221 MiB, потому что Linux ещё не materialized все
страницы `malloc`-буфера pixels; Massif и расчёт allocation дают полный объём.

## Memory breakdown baseline

Для 14 155 776 cells:

| Owner | Layout | Bytes | MiB |
|---|---|---:|---:|
| World cells | 14 155 776 × 16 B | 226 492 416 | 216.00 |
| World pixels | 14 155 776 × 4 B | 56 623 104 | 54.00 |
| Chunk flags | 13 824 × 4 B | 55 296 | 0.05 |
| Lighting | 221 184 × 6 × 4 B | 5 308 416 | 5.06 |
| Итого CPU estimate | плюс `World`/minor | ~288 489 232 | 275.12 |

Одна GPU texture 16384×864 RGBA8 требует ещё примерно 54 MiB VRAM. Во время
`GenImageColor` существует временный CPU image такого же размера, поэтому пик
startup выше steady state.

## Проверенные hypotheses на baseline

- **Подтверждено:** persistent `World.pixels` стоит 54 MiB и используется как
  staging для dirty texture uploads; постоянное full-world хранение не нужно.
- **Подтверждено:** `Cell` в 16 B доминирует в памяти (216 MiB). Менять layout
  можно только с before/after benchmark и regression tests. Закрыто: 12 B,
  162 MiB.
- **Подтверждено как архитектурный limit:** одна texture шириной 16384 зависит
  от GPU maximum texture size и не масштабируется дальше. Закрыто render
  paging-ом: резидентны только видимые страницы 256×256.
- **Частично подтверждено:** sleeping world не обрабатывает cells, но каждый
  tick всё равно сканирует 13 824 chunk flags; это видно по 0,59 ms settled.
  Закрыто active chunk scheduler-ом: settled world стоит 0.000 ms.
- **Не измерено:** цена moving player light, GPU uploads и render preparation.
  Свет измерен и оптимизирован: 10.19 → 1.73 ms на кадр.
- **Подтверждено:** generation eager — каждый reset записывает все 14,1 млн
  cells и занимает около 0,47 s.
- **Подтверждено аудитом:** `world.c` совмещает storage, materials, simulation,
  thermal, generation, effects, lighting и renderer; World владеет GPU state.
  Обе части закрыты: GPU state перенесён в `WorldRenderer`, а `world.c`
  разделён на модули по ответственностям.
- **Подтверждено аудитом:** player, abilities и particles смешивают simulation
  с `Draw*`; gameplay RNG зависит от глобального raylib RNG.

Результаты следующих крупных phases добавляются ниже, а baseline не
перезаписывается.

## После Game/Input/Events boundary

Gameplay orchestration перенесена из `main.c` в `GameUpdate`, raw input — в
`input.c`, transient flags собраны в fixed-capacity `GameEventBuffer`. World hot
loops и layout не менялись. Повторный `make bench` дал:

| Scenario | avg ms | p50 | p95 | p99 | cells/tick | chunks/tick |
|---|---:|---:|---:|---:|---:|---:|
| settled world | 0.565 | 0.498 | 0.854 | 0.945 | 0 | 0 |
| falling sand | 1.874 | 1.608 | 2.899 | 3.540 | 59 630 | 58 |
| large water | 4.129 | 3.705 | 5.849 | 7.002 | 86 664 | 84 |
| fire and lava | 2.267 | 2.042 | 3.493 | 4.062 | 52 929 | 51 |
| large explosion | 1.668 | 1.667 | 2.911 | 6.635 | 44 691 | 43 |
| mass destruction | 0.999 | 0.843 | 1.537 | 2.143 | 23 045 | 22 |
| boost drilling | 1.029 | 0.908 | 1.587 | 1.902 | 27 904 | 27 |
| force ability | 2.132 | 1.815 | 3.419 | 4.099 | 48 372 | 47 |
| cryo ability | 3.933 | 3.508 | 5.630 | 6.602 | 87 648 | 85 |
| chaotic mixed | 5.999 | 5.311 | 9.232 | 14.392 | 139 628 | 136 |

Structural workload совпал с baseline во всех сценариях. Разброс wall-clock
остался обычным для этой машины; ускорение не заявляется. Memory layout и
estimate не изменились.

## После разделения presentation и GPU ownership

`PlayerDraw`, `PowersDrawWorld` и `ParticlesDraw` механически перенесены в
dedicated renderer modules. `Renderer` стал presentation owner, а
`WorldRenderer` — единственным владельцем world `Texture2D`. Из `World`
удалён persistent full-world `Color *pixels`; видимый dirty chunk теперь
строится в stack staging 32×32 (4 KiB) и немедленно загружается.

Memory для production world:

| Owner | Baseline MiB | После MiB | Изменение |
|---|---:|---:|---:|
| Cells | 216.00 | 216.00 | 0 |
| Persistent world pixels | 54.00 | 0.00 | -54.00 |
| Chunk flags | 0.05 | 0.05 | 0 |
| Lighting | 5.06 | 5.06 | 0 |
| CPU estimate | 275.12 | 221.12 | **-54.00 MiB (-19.6%)** |
| Renderer staging | 0 | 0.0039 | stack, не persistent heap |

Полный повторный `make bench`:

| Scenario | avg ms | p50 | p95 | p99 | cells/tick | chunks/tick |
|---|---:|---:|---:|---:|---:|---:|
| settled world | 0.587 | 0.535 | 0.923 | 1.182 | 0 | 0 |
| falling sand | 1.884 | 1.634 | 3.059 | 3.550 | 59 630 | 58 |
| large water | 4.421 | 3.930 | 6.591 | 8.623 | 86 664 | 84 |
| fire and lava | 2.313 | 2.084 | 3.657 | 4.418 | 52 929 | 51 |
| large explosion | 1.665 | 1.667 | 2.814 | 6.750 | 44 691 | 43 |
| mass destruction | 0.971 | 0.893 | 1.259 | 1.986 | 23 045 | 22 |
| boost drilling | 1.059 | 0.919 | 1.633 | 1.961 | 27 904 | 27 |
| force ability | 2.068 | 1.807 | 3.307 | 5.008 | 48 372 | 47 |
| cryo ability | 4.024 | 3.653 | 5.558 | 6.614 | 87 648 | 85 |
| chaotic mixed | 5.930 | 5.353 | 8.736 | 10.365 | 139 628 | 136 |

Init 49.6 ms, generation 427.4 ms, regeneration 459.7 ms; measured RSS and
peak RSS after materializing the world were 220.95/220.87 MiB. Structural
workload снова полностью совпал с baseline. Simulation algorithms не менялись,
поэтому ускорение CPU tick не заявляется; результат этой phase — измеримое
снижение памяти и чистая ownership boundary.

Одна texture 16384×864 всё ещё требует примерно 54 MiB VRAM и остаётся hard
limit текущего renderer. Теперь ограничение локализовано в `WorldRenderer`, так
что render paging не потребует менять `World` или gameplay modules.

## После декомпозиции world.c

`world.c` (2399 строк) разделён на `materials.c`, `world_storage.c`,
`world_simulation.c`, `world_thermal.c`, `world_generation.c`,
`world_lighting.c`, `world_effects.c` и `world_render_data.c` без изменения
алгоритмов. Гипотеза «`world.c` совмещает слишком много responsibilities»
подтверждена и закрыта.

Единственный риск разделения — потеря inlining в горячем цикле: `WorldGetCell`,
`WorldCell`, `WorldInBounds`, `MaterialAt` и `CoordinateHash` вызываются
несколько раз на клетку за tick и раньше жили в одном translation unit.
Поэтому они перенесены в `world_internal.h`/`materials.h` как `static inline`,
а внутренние вызывающие используют `WorldMaterialAt` вместо публичного
`WorldGetCell`. Значение cryo-скорости переехало из `switch` в колонку
`chillRate` таблицы материалов; числа не менялись.

| Scenario | до avg ms | после avg ms | cells/tick |
|---|---:|---:|---:|
| settled world | 0.533 | 0.398 | 0 |
| falling sand | 1.734 | 1.727 | 59 630 |
| large water | 4.294 | 4.131 | 86 664 |
| fire and lava | 2.214 | 2.159 | 52 929 |
| large explosion | 1.634 | 1.534 | 44 691 |
| mass destruction | 0.941 | 0.859 | 23 045 |
| boost drilling | 1.005 | 0.935 | 27 904 |
| force ability | 1.974 | 1.872 | 48 372 |
| cryo ability | 4.218 | 3.813 | 87 648 |
| chaotic mixed | 5.819 | 5.639 | 139 628 |

Оба замера сделаны в одной сессии на одной машине. Структурные счётчики
совпали до единицы во всех десяти сценариях: активные/спящие chunks,
`cells/tick`, `chunks/tick` и `dirty/tick` идентичны, то есть работа не
изменилась. Wall-clock во всех сценариях не хуже прежнего; регрессии от
разделения нет. Ускорение не заявляется — разброс этой машины сопоставим с
разницей, а алгоритмы не менялись. Память не изменилась: estimate 221.12 MiB,
RSS 220.96 MiB.

## После детерминированного RNG

Gameplay больше не берёт из process-wide генератора raylib. Генерация мира,
мировые эффекты, powers и particles получили собственные потоки SplitMix64,
выведенные из одного seed. `WorldGenerate(world, seed)` теперь принимает seed
явно, `GameConfig.seed` задаёт seed сессии, а `--seed VALUE` и строка `SEED` в
debug HUD делают любой мир воспроизводимым.

**Это создаёт новый structural baseline, а не regression.** Benchmark
генерирует мир другим генератором, значит рельеф под сценариями другой, и
`cells/tick` сравнивать со строками выше нельзя. Числа до этой phase
сопоставимы между собой, числа начиная с этой — между собой.

| Scenario | avg ms | p50 | p95 | p99 | cells/tick | chunks/tick |
|---|---:|---:|---:|---:|---:|---:|
| settled world | 0.391 | 0.376 | 0.475 | 0.551 | 0 | 0 |
| falling sand | 1.519 | 1.413 | 1.940 | 2.687 | 47 627 | 46 |
| large water | 3.882 | 3.643 | 4.996 | 6.814 | 74 660 | 72 |
| fire and lava | 2.094 | 1.892 | 3.108 | 3.518 | 40 925 | 39 |
| large explosion | 1.295 | 1.427 | 1.792 | 6.685 | 32 375 | 31 |
| mass destruction | 0.658 | 0.574 | 0.850 | 1.415 | 11 042 | 10 |
| boost drilling | 0.687 | 0.640 | 0.940 | 1.185 | 15 900 | 15 |
| force ability | 1.632 | 1.486 | 2.071 | 4.183 | 36 369 | 35 |
| cryo ability | 3.691 | 3.487 | 4.710 | 6.877 | 75 645 | 73 |
| chaotic mixed | 5.426 | 5.297 | 6.743 | 8.247 | 127 624 | 124 |

Init 71.2 ms, generation 370.0 ms, regeneration 346.3 ms. Память не изменилась:
estimate 221.12 MiB, RSS 220.89 MiB. `Rng` — 8 bytes на систему, вне cell
storage.

Гипотеза «gameplay RNG мешает reproducibility» подтверждена и закрыта:
`test_a_seeded_session_replays_identically` прогоняет 40 фиксированных кадров с
лазером, взрывом, силовым ударом и криолучом в двух независимых `GameState` и
требует побитового совпадения позиции игрока и digest мира.

## После разделения abilities и particles

`PowerSystem` заменён реестром `AbilitySystem`: общая часть (trigger, cooldown,
follow-through, выбранная способность) принадлежит драйверу, специфика — одной
функции `apply` на способность. Отдача уходит к игроку через
`GameEvent.playerImpulse`, поэтому `PlayerApplyExplosionImpulse` удалена, а
`Player` больше не перечисляет существующие powers. Частицы остались одним
fixed-capacity пулом, но visual-роль теперь обновляется через `const World *` и
не может изменить клетку.

Симуляция мира не менялась. Структурные счётчики совпали со всеми десятью
сценариями предыдущего запуска до единицы:

| Scenario | avg ms | cells/tick | chunks/tick |
|---|---:|---:|---:|
| settled world | 0.441 | 0 | 0 |
| falling sand | 1.484 | 47 627 | 46 |
| large water | 3.970 | 74 660 | 72 |
| fire and lava | 2.013 | 40 925 | 39 |
| large explosion | 1.281 | 32 375 | 31 |
| mass destruction | 0.718 | 11 042 | 10 |
| boost drilling | 0.681 | 15 900 | 15 |
| force ability | 1.664 | 36 369 | 35 |
| cryo ability | 3.697 | 75 645 | 73 |
| chaotic mixed | 5.548 | 127 624 | 124 |

Память не изменилась: estimate 221.12 MiB, RSS 220.90 MiB. `AbilitySystem` —
одна структура на игрока, вне cell storage.

Гипотезы «ability simulation смешана с presentation» и «particles смешивают
gameplay и visual effects» подтверждены и закрыты.

## После active chunk scheduler

Гипотеза «simulation всё ещё перебирает слишком много inactive chunks»
подтверждена и закрыта. `WorldUpdate` перебирал chunk-сетку целиком: для
каждой из 864 строк он проходил все 512 колонок, то есть около 442 000 слотов
за tick, даже когда весь мир спал.

Расписание теперь хранится в двух представлениях: флаг на chunk для проверки
принадлежности за O(1) и компактный список активных колонок на каждую строку
chunk-ов для обхода. Списки заполняются только пробуждениями, поэтому спящая
строка не стоит ничего. Порядок обхода сохранён точно: перед проходом строка
сортируется вставками, поэтому она по-прежнему идёт слева направо или справа
налево, а не в порядке поступления пробуждений.

Одно поведенческое изменение зафиксировано намеренно: множество, симулируемое
в этом tick, заморожено в начале `WorldUpdate`, поэтому пробуждение внутри tick
планирует **следующий** tick. Раньше chunk, разбуженный по ходу tick и ещё не
посещённый, мог отработать в том же tick, а разбуженный ниже по обходу — нет;
это была асимметрия, а не инвариант. `updatedTick` по-прежнему гарантирует одно
перемещение клетки за tick, а пробуждение — что chunk отработает. Видно это
только как сдвиг счётчиков на доли процента в сценариях с огнём и лавой.

| Scenario | до avg ms | после avg ms | изменение |
|---|---:|---:|---:|
| settled world | 0.441 | **0.000** | практически исчез |
| falling sand | 1.484 | 1.362 | −8% |
| large water | 3.970 | 3.807 | −4% |
| fire and lava | 2.013 | 1.562 | −22% |
| large explosion | 1.281 | 1.397 | шум p99 |
| mass destruction | 0.718 | 0.316 | −56% |
| boost drilling | 0.681 | 0.298 | −56% |
| force ability | 1.664 | 1.529 | −8% |
| cryo ability | 3.697 | 3.435 | −7% |
| chaotic mixed | 5.548 | 5.536 | −0% |

Выигрыш тем больше, чем меньше активная доля мира, — то есть именно в обычной
игре. Сценарии, где реальная работа с клетками доминирует (water, chaotic),
почти не изменились: там перебор сетки и раньше был малой долей.

Побочно исчезли ещё два полных прохода по сетке за tick: `memset` буфера
next-active и объединение active-флагов в dirty/light-dirty теперь идут по
списку. `activeChunkCount` поддерживается инкрементально, поэтому он точен и
между tick-ами — раньше лазер между двумя tick-ами оставлял HUD с устаревшим
числом.

Память выросла на 4 массива расписания: `chunkCount × 4 B × 2` плюс
`chunkRows × 4 B × 2` — около 0.11 MiB на production world. Estimate 221.12 MiB,
RSS 221.00 MiB.

## После сжатия Cell до 12 bytes

Гипотеза «`Cell` в 16 B доминирует в памяти» подтверждена и закрыта. `Cell`
уплотнена до 12 bytes перестановкой полей и сужением обоих счётчиков-штампов
до 16 bit:

```c
typedef struct Cell {
    float temperature;   /* 0..3  */
    uint16_t updatedTick;/* 4..5  */
    uint16_t effectStamp;/* 6..7  */
    uint16_t lifetime;   /* 8..9  */
    uint8_t material;    /* 10    */
} Cell;                  /* 12, один байт padding */
```

Оба штампа сравниваются на равенство со счётчиком мира, то есть осмысленны
только по модулю своей ширины. Цена сужения ограничена и измерима: клетка, чей
штамп случайно совпал с усечённым счётчиком, пропускает ровно один tick или
ровно один эффект и дальше ведёт себя нормально. Для `updatedTick` это требует,
чтобы клетка простояла бодрствующей и нетронутой ровно кратное 65 536 tick-ов —
восемнадцать минут — и стоит ей одного кадра падения.

Единственная популяция, для которой это было бы заметно, — никогда не
записанные клетки со штампом 0. Поэтому оба счётчика пропускают значение 0:
`WorldUpdate` увеличивает tick ещё раз, если усечённое значение обнулилось, а
`WorldNextEffectStamp` делает то же для серии эффектов.
`test_the_tick_counter_survives_wrapping_its_cell_stamp` прогоняет мир через
переполнение и требует сохранения массы песка и продолжения падения.

Память для 14 155 776 cells:

| Owner | Baseline MiB | Сейчас MiB | Изменение |
|---|---:|---:|---:|
| Cells (16 B → 12 B) | 216.00 | 162.00 | -54.00 |
| Persistent world pixels | 54.00 | 0.00 | -54.00 |
| Chunk metadata и расписание | 0.05 | 0.16 | +0.11 |
| Lighting | 5.06 | 5.06 | 0 |
| **CPU estimate** | **275.12** | **167.22** | **-107.90 MiB (-39%)** |
| Измеренный RSS | ~275 | 167.00 | |

| Scenario | до avg ms | после avg ms | cells/tick |
|---|---:|---:|---:|
| settled world | 0.000 | 0.000 | 0 |
| falling sand | 1.362 | 1.310 | 47 627 |
| large water | 3.807 | 3.976 | 74 660 |
| fire and lava | 1.562 | 1.542 | 40 772 |
| large explosion | 1.397 | 0.912 | 32 375 |
| mass destruction | 0.316 | 0.265 | 11 042 |
| boost drilling | 0.298 | 0.374 | 15 900 |
| force ability | 1.529 | 1.419 | 36 363 |
| cryo ability | 3.435 | 3.370 | 75 593 |
| chaotic mixed | 5.536 | 5.530 | 127 414 |

Структурные счётчики совпали во всех сценариях. Wall-clock в пределах разброса
машины; ускорение не заявляется, хотя `WorldInit` ожидаемо стал вдвое дешевле
(68 → 36 ms), потому что нулевой инициализации подлежит на 54 MiB меньше.

## После render paging

Гипотеза «одна texture шириной 16384 — плохое long-term limitation»
подтверждена и закрыта. Единственная world-texture заменена кэшем страниц
256×256 cells: резидентны только те страницы, которые видит камера.

| | До | После |
|---|---|---|
| Texture objects | 1 × 16384×864 | до 12–24 × 256×256 |
| VRAM для world | ~54.0 MiB, всегда | 0.25 MiB на резидентную страницу |
| Типичный кадр (окно 1280×720) | 54.0 MiB | 4 видимых страницы, 12 слотов = 3.0 MiB |
| Предел ширины мира | `GL_MAX_TEXTURE_SIZE` | память и симуляция |

Кэш растёт под размер вида (окно + zoom) и не уменьшается, поэтому resize
выделяет текстуры один раз, а установившийся рендер не аллоцирует ничего.
Вытеснение — LRU, при этом страница, уже заявленная в этом кадре, вытеснена
быть не может. Панорамирование стоит одной загрузки страницы на переднем крае
вместо чего-либо, пропорционального карте.

Важная деталь корректности: `WorldRenderChunkVisitor` теперь возвращает
`bool`. Chunk, который renderer не смог разместить, **сохраняет** свой dirty
flag и перестраивается в кадре, когда его страница станет резидентной. Иначе он
был бы тихо потерян, и на экране остались бы устаревшие пиксели до следующего
случайного изменения этого chunk — баг, который проявился бы только на машине с
меньшим кэшем страниц, чем у разработчика.
`test_a_refused_chunk_keeps_its_dirty_flag` закрывает это.

Симуляция не менялась; benchmark headless и страниц не создаёт. CPU estimate и
RSS прежние: 167.22 / 167.00 MiB.

## После локализации lighting

`make bench` теперь измеряет и свет: `WorldUpdateLighting` работает целиком на
CPU, GL-контекст ему не нужен. Первое же измерение подтвердило гипотезу
«moving player light вызывает слишком много lighting work» — и с большим
запасом:

| Режим | До | После |
|---|---:|---:|
| ничего не изменилось | 0.022 ms | 0.013 ms |
| движущийся фонарь игрока | **10.189 ms** | **1.732 ms** |
| p95 движущегося фонаря | 11.612 ms | 2.340 ms |
| бурение (терраформинг каждый кадр) | 10.108 ms | 1.649 ms |

10 ms на кадр — это больше всего бюджета кадра при 120 FPS, и платились они
каждый раз, когда игрок сдвигался хотя бы на 4 cells, то есть в полёте —
каждый кадр. Причина: solve шёл по всему полю света 2048×108, при том что
игрок видит около 80 колонок из 2048.

Solve теперь ограничен окном колонок: видимая область плюс запас
`WORLD_LIGHT_WINDOW_MARGIN = 128` световых cells. Открытый воздух пропускает
0.97 на световую cell, поэтому источник за пределами окна приходит к видимому
краю с коэффициентом 0.97¹²⁸ ≈ 2% — ниже шага квантования, с которым сравнивает
renderer, и ниже того, что вообще может показать байт на канал. Sky-канал окно
не портит принципиально: он заполняется проходом по колонке сверху, а колонка
решается независимо от соседей.

Окно входит в условие пересолвинга: если оно сдвинулось, колонки, которых
прошлый solve не касался, устарели.

**Отвергнуто измерением:** разделение каналов. Идея выглядела правильной — sky
меняется только вместе с рельефом, поэтому движущийся фонарь мог бы solve-ить
только ember. Реализовано и измерено: полёт выиграл 0.2 ms, а бурение
подорожало с 1.6 до 3.2 ms, потому что каналы делят между собой поиск
transmission и весь расчёт индексов, а бурение — основной глагол этой игры.
Разделение откачено, причина записана в комментарии рядом с `WorldSolveLight`,
чтобы её не «улучшили» обратно.

Заодно удалён `WorldLightAt`: публичная функция, которую никто не вызывал.

Остаточный долг: `WorldUpdateLighting` всё ещё сканирует все 13 824 флага
light-dirty за кадр (0.013 ms). Это следующий кандидат на компактный список —
тот же приём, что и в active chunk scheduler.

## Итог миграции и оставшиеся hotspots

Финальный `make bench` на той же машине (Intel Core i5-6300U, Linux x86_64,
GCC 16.1.1, raylib 6.0.0, `-O2`):

| Scenario | baseline avg ms | итог avg ms | примечание |
|---|---:|---:|---|
| settled world | 0.594 | **0.000** | active chunk scheduler |
| falling sand | 2.042 | 1.216 | |
| large water | 4.333 | 3.668 | |
| fire and lava | 2.678 | 1.666 | |
| large explosion | 1.727 | 0.841 | |
| mass destruction | 0.998 | 0.242 | |
| boost drilling | 1.050 | 0.286 | |
| force ability | 2.138 | 1.482 | |
| cryo ability | 3.966 | 3.174 | |
| chaotic mixed | 5.864 | 5.152 | |
| lighting, движущийся фонарь | 10.189 | **1.677** | |

Строки после «детерминированного RNG» несопоставимы со строками до него по
`cells/tick`: генератор рельефа сменился, значит сценарии стоят на другой
земле. Порядок величины и относительные изменения внутри каждой серии
сопоставимы.

Память: **275.12 → 167.22 MiB** CPU estimate (−39%), измеренный RSS
167.02 MiB. VRAM для мира: **~54 MiB постоянно → 0.25 MiB на резидентную
страницу**, обычно 3 MiB.

### Профиль оставшегося (gprof, 90 ticks × 10 сценариев)

| Функция | % | Что это |
|---|---:|---|
| `WorldGenerate` + `WorldSetGeneratedCell` | 37.6 | генерация мира, **не** кадровая стоимость |
| `WorldTryMaterialReaction` | 10.9 | скан 8 соседей у каждой water/lava клетки за tick |
| `WorldWakeCellAndNeighbors` | 8.5 | путь пробуждения |
| `WorldUpdateLighting` | 8.2 | 1.1 ms на solve |
| `WorldUpdate` (сам обход) | 7.4 | |
| `WorldHeatNeighbors` | 3.7 | |

Кадровый профиль плоский: после устранения архитектурных потерь ни одна функция
не доминирует, и оставшееся — это настоящая работа с клетками. Дальнейшая
микрооптимизация даст единицы процентов и здесь сознательно не делалась.

Главная оставшаяся цель — **генерация**: 286 ms на полный мир, целиком eager.
Это не стоимость кадра, но это стоимость каждого нажатия `R` и потолок для
миров больше текущего. Правильное решение — не микрооптимизация, а
детерминированная генерация по chunk (см. «Оставшийся долг» ниже).

### Оставшийся долг

1. **Eager generation.** `WorldGenerate` пишет все 14.1 млн клеток за 286 ms.
   Детерминированная генерация по chunk (`GenerateChunk(seed, chunkX, chunkY)`
   на координатном хэше вместо последовательного RNG) убрала бы и паузу, и
   потолок размера мира. Seed-инфраструктура для этого уже готова.
2. **Chunk-owned storage.** Клетки лежат одним плоским массивом. Для выгрузки
   дальних регионов и save/load нужен массив на chunk. Это трогает каждое
   обращение к клетке, поэтому — отдельная phase.
3. **Скан light-dirty флагов.** `WorldUpdateLighting` проходит все 13 824 флага
   за кадр (0.013 ms). Просится тот же компактный список, что и в scheduler.
4. **`WorldTryMaterialReaction`.** 10.9% профиля на скан, который почти всегда
   ничего не находит: у большинства клеток воды рядом нет лавы. Дешёвый
   предварительный признак на chunk («в этом chunk есть и вода, и лава») убрал
   бы почти весь скан.
5. **Renderer benchmark.** GPU uploads и render preparation измеряются только
   runtime-счётчиками в HUD; воспроизводимого headless-замера для них нет,
   потому что он требует GL-контекста.

## EF-RND-002 — explicit emissive bloom

Замеры 2026-08-31 выполнены `xvfb-run` на Mesa llvmpipe 26.1.5 при 1280×720,
после цикла resize 1280×720 → 960×540 → 1280×720. Два прогона стабильных
кадров после восстановления размера дали `bloomSubmissionMilliseconds` avg
**13.075–15.912 ms**, наибольший max **17.320 ms**. Это CPU submission interval
вокруг emissive/downsample/blur; на software renderer он включает rasterization
и не является оценкой времени обычной GPU. Headless `make bench` намеренно не
измеряет эти passes.

Структурная стоимость кадра с bloom:

- 5 offscreen passes: sharp scene, explicit emissive, half-resolution
  downsample/threshold, horizontal blur, vertical blur;
- 4 `RenderTexture2D`: два 1280×720 и два 640×360;
- final backbuffer composite: один sharp draw и один additive bloom draw;
- steady-state resource allocations: 0.

Memory/upload delta:

| Ресурс | До | После |
|---|---:|---:|
| Render-target VRAM, RGBA8 + 32-bit depth | ~14.06 MiB | ~17.58 MiB |
| Default 12-slot world page cache | ~3.00 MiB | ~6.00 MiB |
| Chunk stack staging | 4 KiB | 8 KiB |
| Persistent world CPU estimate | 167.22 MiB | 167.22 MiB |

Каждый dirty chunk теперь делает два `UpdateTextureRec` и передаёт 8 KiB вместо
4 KiB; settled chunks по-прежнему не загружаются. Новый headless regression
проверяет, что lava/fire входят в mask, bright sand не входит, а particle glow
задаётся явно. Xvfb screenshot подтвердил резкий terrain, Y-orientation и
controlled glow у explosion particles и отдельного lava/fire fixture.

Если shader source отсутствует или не компилируется, либо half-resolution
targets не выделились, renderer сохраняет sharp scene path. Запуск из `/tmp`
без `assets/shaders/` подтвердил fallback без crash; текущий asset lookup
относителен к working directory и остаётся packaging-ограничением.

## EF-FX-001 — bounded presentation FX

Замер 2026-09-01: `sizeof(PresentationFx) == 56`, полный встроенный pool на 128
instances вместе с telemetry занимает **7176 bytes (7.01 KiB)**. Heap
allocation отсутствует; update и каждый draw pass ограничены 128 элементами, а
event conversion — существующим лимитом 256 `GameEvent` на frame.

`make bench` сохранил production world estimate **167.22 MiB** и те же
structural workload counters во всех десяти scenarios: presentation manager не
входит в `GameState` и headless simulation loop. Runtime HUD публикует
active/peak/dropped FX; Xvfb smoke для одного explosion получил `fx_peak=2`,
`fx_dropped=0`. Отдельного wall-clock speedup не заявляется: задача добавляет
bounded presentation work, а headless benchmark не rasterizes его geometry.

### EF-VFX-001 budget delta

Замер 2026-09-01 после combat presentation pass:
`sizeof(PresentationFx) == 60`, весь 128-slot manager вместе с cooldown/RNG
state занимает **7720 B (7.54 KiB)**. Capacity не менялась. Отдельный
16-slot `CameraFeedback` занимает **532 B** (`CameraImpulse == 32 B`). Оба
цикла жёстко ограничены своими capacities и не выделяют память в normal frame.

Расширенный Xvfb smoke последовательно вызвал boost I, laser, explosion, force
и cryo через `GameInput`: `fx_peak=48`, `fx_dropped=0`, bloom остался 5 passes /
4 targets на 640×360 half-resolution target при окне 1280×720. Render passes,
target resolutions, texture upload path и FX capacity не менялись; CPU
`make bench` не является измерением этих draw primitives, поэтому новый
wall-clock speedup/slowdown по headless simulation не заявляется.

## EF-DYN-005 — TerrainBody renderer cache

Замер 2026-09-01: `sizeof(TerrainBodyRenderer) == 67 480` B, из которых 64 KiB
— постоянная scene/emissive staging pair максимального raster; один cache slot
занимает 60 B. Cache фиксирован тем же `MAX_TERRAIN_BODIES == 32`, что и
simulation, и в steady state не выполняет heap/GPU allocations.

Worst-case texture pixels:

```text
32 bodies × 8192 texels × 2 RGBA8 layers = 2.00 MiB
```

Texture создаются exact-size, поэтому фактический smoke body 12×6 занял 576 B
(HUD округляет до 0.6 KiB). Первый GL frame сделал ровно два uploads, следующие
unchanged frames — ноль; scene+emissive дали два draw calls, а smoke
зафиксировал contact body с static shelf. После free cache в
том же render frame сообщил 0 bodies и 0 bytes. Resize offscreen targets не
пересоздавал body textures.

Normal-frame CPU work структурно ограничено проходом по 32 slots дважды
(scene/emissive culling) плюс draw видимых bodies. Raster обходится только при
новом generation или изменении `rasterRevision`; в последнем случае textures
обновляются in-place. Headless `make bench` не rasterizes raylib geometry,
поэтому wall-clock speedup/overhead для GPU здесь не заявляется; benchmark
нужен для подтверждения неизменности simulation workload counters.

## EF-ENV-001 — procedural environment budget

`EnvironmentRenderer` не добавляет texture, shader или render pass. Он хранит
47 фиксированных descriptors по 20 B (940 B) и небольшой state/stats block —
около 1 KiB CPU суммарно. Runtime generation происходит только при init/смене
world seed; steady-state heap allocations, texture uploads и world scans равны
нулю.

Xvfb smoke при 1280×720 показал фиксированный максимум **76 scene + 11
emissive primitive submissions**, все три palettes (`env_mask=0x7`), корректный
resize, transient rotation/zoom feedback и принудительный high-speed zoom-out.
Pipeline остался на 5 offscreen passes / 4 targets, bloom — 640×360. llvmpipe
submission timing сильно зависит от software rasterizer и приведён только как
информационный: два полных актуальных smoke дали 24.6..25.8 ms average и
56.2..56.9 ms maximum.

Headless simulation benchmark не rasterizes environment. Его counters должны
оставаться неизменными, потому что `EnvironmentRenderer` не входит в
`GameState`; отдельный wall-clock renderer speedup/slowdown не заявляется.
## EF-WLD-001 — biome generation v1

Замер 2026-09-01 выполнен на production world 16384×864 одним `make bench` до
изменения и одним после в том же worktree. Генератор остался eager и не изменил
memory layout: CPU estimate **167.22 MiB**, measured RSS **166.84 MiB** после.

| Startup operation | До | После |
|---|---:|---:|
| `WorldGenerate` | 306.569 ms | 259.762 ms |
| regeneration | 335.892 ms | 256.341 ms |

Это уменьшение времени, а не устранение паузы reset: все 14,1 млн cells всё ещё
записываются. Выигрыш получен несмотря на четыре biome profiles и новые surface
features, потому что cave/liquid descriptors теперь масштабируются по ширине, а
strata hash вычисляется один раз на колонку. Дополнительная постоянная память и
heap allocations отсутствуют.

Wall-clock и workload counters десяти gameplay scenarios после смены terrain
**не сравниваются** с прежними: benchmark arena стоит в другом biome и её края
соседствуют с другим набором generated dynamics. Settled contract сохранился:
0 active chunks, 0 processed cells. Архитектурный долг остаётся прежним —
coordinate-seeded descriptors уже пригодны для chunk generation, но storage и
lifecycle пока eager, поэтому следующий шаг нельзя выдавать за сделанный.

## EF-WLD-002 — плавные переходы и темнота

Замер 2026-09-01 на production world 16384x864, `make bench` на этой же машине
до и после изменения. Разброс между прогонами велик, поэтому цифры приведены
как несколько выборок, а не как одно число.

| Startup operation | До | После |
|---|---:|---:|
| `WorldGenerate` | 352.7 ms | 385.6 / 362.3 / 371.2 ms |
| regeneration | 466.7 ms | 364.9 / 375.7 / 355.9 ms |

Генерация подорожала примерно на 5-6%. Причина одна: полоса смешивания выросла
с четверти ширины региона до половины, поэтому поклеточное решение «какому из
двух биомов принадлежит клетка» теперь принимается для вдвое большего числа
колонок. Решётка гладкого поля вынесена из поклеточного пути и считается по
одному разу на колонку; без этого та же правка стоила примерно вдвое дороже.
Постоянной памяти и heap-аллокаций не добавилось: `BiomeBlendColumn` — 96 float
на стеке цикла генерации.

Темнота не стоит ничего. Решатель света не изменился ни на операцию: поменялись
две константы и то, как результат превращается в цвет, а вуаль воздуха берёт уже
посчитанный канал неба вместо прежней арифметики по глубине.

## EF-RND-003 — рацион на перестроение чанков (откачен)

Жалоба была конкретной: на верхней ступени полёта кадры стабильно падают с 60
до 40. Предположение пользователя — «возможно проблема в скорости прогрузки
мира» — оказалось верным, но принятое лечение оказалось хуже болезни и **было
откачено**.

Мерить это по HUD нельзя: одиночный кадр на нагруженной машине разбрасывается в
несколько раз и уводит настройку не в ту сторону (одна и та же сборка дала 3.82
и 11.98 мс подряд). Поэтому smoke-прогон усредняет
`WorldRendererStats::preparationMilliseconds` по всей фазе полёта и печатает
`prepare_avg` и `prepare_max`. **Этот измеритель оставлен** — он и есть главное,
что осталось от попытки.

| Бюджет чанков на кадр | prepare_avg | prepare_max |
|---|---:|---:|
| **без ограничения (текущее состояние)** | **8.891 ms** | **35.810 ms** |
| 32 | 6.861 ms | 19.088 ms |
| 64 | 4.417 ms | 27.637 ms |
| 128 | 4.625 / 6.642 ms | 19.849 / 24.128 ms |

Мир прибывает комками: страница, въезжающая в кадр, помечает грязными все свои
64 чанка разом, световой решатель помечает ещё, каждый раз когда сдвигается его
окно, а чанк — это тысяча клеток пересчёта пикселей с билинейной выборкой света
на каждую. Рацион вдвое уменьшал среднюю стоимость кадра — и ровно поэтому
страницы, въезжающие в кадр, появлялись **чёрными квадратами** и дозаполнялись
за кадр-другой. Видимый артефакт подгрузки хуже лишних миллисекунд, поэтому
рацион снят и поведение вернулось к прежнему.

**Что ещё было измерено и отвергнуто.** Захват страниц на кадр вперёд оказался в
несколько раз *хуже*: решатель света и пометка перевысвеченных чанков идут по
тому же прямоугольнику, и его расширение стоило намного больше, чем выигрывала
ранняя загрузка (11–15 мс на кадр при нуле новых страниц). Уменьшение страницы
со 256 до 128 тоже не окупилось: всплеск при въезде вчетверо меньше, но
постоянных страниц вчетверо больше.

**Что остаётся сделать.** Правильное направление — не размазывать работу по
кадрам, а сделать её дешевле: перестроение чанка это тысяча вызовов
`MaterialPixel` с билинейной выборкой света, и там есть что убрать. Любое
решение, при котором игрок видит незаполненную геометрию, заведомо не подходит.

## Замер 2026-09-02: мир стал выше

Высота мира выросла с 864 до 1440 клеток (`DEFAULT_WORLD_HEIGHT`), и почти весь
прирост ушёл в небо: поверхность опущена так, что грунт стал глубже примерно на
треть, а подъём от земли до космоса вырос с ~250 клеток до ~640. `make bench`
теперь измеряет ту же карту, что и игра, — держать бенчмарк на мире, которого
больше нет, значит не измерять ничего, — поэтому все числа ниже не сравнимы с
таблицами выше напрямую.

| Метрика | 16384×864 | 16384×1440 |
| --- | --- | --- |
| `cells` | 162.0 MiB | 270.0 MiB |
| `lighting` | 5.06 MiB | 8.44 MiB |
| `estimated` | 167.2 MiB | 278.7 MiB |
| `final_peak_rss` | 336 MiB | 556 MiB |
| `chaotic mixed` avg | 5.18 ms | 5.00 ms |
| `lighting moving_light` avg | 1.63 ms | 2.69 ms |
| `lighting moving_light` p95 | 2.03 ms | 3.21 ms |
| `traversal` avg | 0.40 ms | 0.41 ms |

Симуляция не подорожала: она ходит по активным chunk-ам, а их число зависит от
того, что происходит, а не от того, сколько мира лежит вокруг. Подорожал **свет
при движущемся источнике** — с 1.63 до 2.69 мс, ровно во столько раз, во сколько
выросло число световых клеток (221 184 → 368 640). Причина в том, что решатель
окном ограничен только по горизонтали: два растровых прохода идут по всем
строкам светового поля, а строк стало в 1.67 раза больше.

Вертикальное окно здесь не поможет тем же приёмом, что горизонтальное. Запас в
128 световых клеток взят из того, что открытый воздух пропускает 0.97 за клетку,
и `0.97^128 ≈ 2%`; при высоте 1440 это 180 строк, то есть весь мир. Настоящая
возможность в другом: **вниз** от окна свет идёт почти целиком сквозь породу с
пропусканием 0.66 и умирает за полтора десятка световых клеток, так что нижняя
граница окна могла бы быть куда ближе верхней. Это отдельная работа по
освещению, и делать её надо так, чтобы игрок не увидел ни одной клетки с
устаревшим светом.

Бюджет при этом остаётся с запасом: 5.0 мс симуляции плюс 2.7 мс света — это
меньше половины кадра в 16.7 мс.

## Замер 2026-09-20: мир, который засыпает (EF-PERF-001)

Жалоба: «при долгой игре FPS сам по себе падает». `make bench` этого не видел,
потому что его сценарии никогда не активируют сгенерированные озёра и кальдеры.
Headless-проба, которая проводит окно активации (960×576) по всей карте, как это
делает долгий полёт, ничего не трогая, и затем даёт миру 3600 ticks покоя:

| | active chunks | cells/tick |
|---|---:|---:|
| сразу после облёта | 1261 | — |
| через 60 ticks, до | 109 | 112 640 |
| через 3600 ticks, **до** | **88** (50 лава, 25 вода, 12 соседи лавы) | **90 112** |
| через 3600 ticks, **после** | **0** | **0** |

Машина: Intel Core i5-1140G7, GCC, `-O2`. Девяносто тысяч клеток за tick — это
порядка 3–4 ms на каждый кадр до конца сессии, плата за один облёт карты; каждый
следующий пожар и разлив добавлял к ней. Две причины, обе — вечное движение,
которого не видно:

- **обод лавового озера.** Rock у лавы получал +3 °C за tick и остывал на 4:
  пила, которая никогда не сходится. Теперь источник с потолком *держит*
  соседа (`Cell.heatHeld`, байт из padding), обод стоит ровно на потолке 660 °C
  и не меняется.
- **береговая линия и зерно над озером.** Поверхностная клетка скользила на
  берег и обратно каждый tick; зерно над ровной поверхностью каталось по ней
  вечно. Теперь поверхностная клетка (над которой пусто) берёт провал, скользит
  только по жидкости и не более `WORLD_LIQUID_WANDER_LIMIT` (60) раз подряд.

`make bench` после изменения: структурные счётчики совпали с точностью до
процента (fire and lava: 34 503 → 32 546 cells/tick — обод больше не будит
соседей; force: 29 752 → 29 343), wall-clock в пределах разброса машины.
Регрессии: `test_a_settled_lava_pocket_lets_its_chunks_sleep`,
`test_a_level_pool_and_its_shore_go_to_sleep`.

## Замер 2026-09-20: свет решается только когда изменились его входы (EF-PERF-002)

Headless-проба (`WorldUpdateLighting` + `WorldPrepareVisible` с пустым
visitor, окно 426×240 клеток, 240 кадров, seed `0x00e6be11`), персонаж висит
на месте и не двигается:

| | light avg | rebuild avg | chunks/frame |
|---|---:|---:|---:|
| до | 2.80 ms | 1.22 ms | 44.8 |
| после | 0.74 ms | 0.62 ms | 28.3 |

Причина: каждый активный chunk помечал свои световые входы грязными каждый tick,
а `terrainChanged` означало «что-то помечено», а не «что-то изменилось». Два
активных chunk-а у точки появления — и полный solve шёл шестьдесят раз в
секунду в неподвижной сцене. Теперь `WorldRefreshLightBlock` сообщает, изменились
ли emission/opacity хотя бы одного блока (точное сравнение — блок пересчитывается
из тех же клеток тем же способом), и solve запускается только тогда.

Второе: небо сеялось *неквантованным* daylight, который сдвигается на каждом
tick, поэтому любой solve по другой причине переворачивал все сэмплы у границ
квантования и рассыпал перезасветку по всему экрану. Seed теперь квантован тем же
шагом `WORLD_LIGHT_STEPS`, и `dayChanged` — это смена шага, а не допуск.

Остаток в неподвижной сцене — это утренний рассвет: daylight пересекает шаг
1/512 каждые ~5 кадров, и каждый раз перезасвечивается весь видимый экран
(~150 chunks). Это следующая цель — она уходит вместе с переносом освещения на
GPU.

## Замер 2026-09-20: свет ушёл на GPU (EF-PERF-003)

Жалоба «на верхней ступени полёта кадры падают с 60 до 40» (см. EF-RND-003, где
рацион на перестроение был откачен). Причина названа там же: перестроение
chunk-а — тысяча вызовов `MaterialPixel` с билинейной выборкой света на каждую
клетку, а лампа игрока, сдвигаясь каждый кадр полёта, перезасвечивала и
перестраивала все chunks, до которых дотягивалась. Поворот дня делал то же со
всем экраном каждые несколько кадров на рассвете.

Свет теперь **применяется на GPU**: страницы хранят неосвещённый цвет,
`LightRenderer` загружает решённое окно поля в текстуру `2048×180` RGBA8
(1.4 MiB VRAM, загружается только видимый регион — единицы KiB) и шейдер
`world_light.fs` освещает страницы и отделённые тела по мировой позиции
фрагмента. Chunk перестраивается только когда в нём изменилась клетка.

Smoke-прогон на реальном GPU (Intel Iris Xe, 1280×720, усреднение по фазе
полёта), одна и та же машина, одна сессия:

| | до | после |
|---|---:|---:|
| `prepare_avg` (страницы) | **7.65 ms** | **0.27 ms** |
| `prepare_max` | 26.8 ms | 6.3 ms |
| solve света, `moving_light` (`make bench`) | 2.41 ms | 1.23 ms |
| `light_avg` в полёте (solve + upload) | — | 2.55 ms |
| bloom submit avg | 0.46 ms | 0.80 ms |

Solve подешевел вдвое сам по себе: ушли поклеточная квантизация (два `floorf`
через libm на сэмпл) и `WorldMarkRelitChunks`. Bloom подорожал на треть,
потому что emissive-плоскость теперь непрозрачна там, где непрозрачна сцена
(см. ниже), и страницы рисуются в неё все, а не только светящиеся; это цена
корректной окклюзии, и она мала.

Картинка попиксельно совпала с прежней: средняя разница по шести reference
screenshots 0.18–0.37 из 255 на канал, пикселей с разницей больше 40 — 0.01%.
Тела теперь освещены по месту, а не постоянным 0.72; на поверхности разница не
видна.

Память: `lightShownSky/Ember` удалены (−2.8 MiB), estimate 278.7 → 275.9 MiB.

Попутно исправлена окклюзия emissive-плоскости: негорящий материал в ней
непрозрачно чёрный, воздух несёт ту же вуаль, что в сцене, герой рисует свой
силуэт. Звёзды больше не «отражаются» на герое и на кусках породы в космосе —
раньше их bloom складывался поверх всего, что нарисовано после них.

Остаток стоимости мира в полёте — solve света (2.5 ms с загрузкой). Это
следующая цель.

## Замер 2026-09-20: solve света вчетверо дешевле (EF-PERF-004)

Headless-проба: лампа движется на 380 cells/s, окно 426×240, 300 solve подряд,
seed `0x00e6be11`, Intel Core i5-1140G7:

| Шаг | solve avg |
|---|---:|
| после EF-PERF-003 (без квантизации и MarkRelit) | 1.17 ms |
| засев неба построчно + вертикальная/горизонтальная части прохода разделены | 1.08 ms |
| оба канала в одной цепочке | 0.78 ms |
| `-fvect-cost-model=cheap` (интерьер вертикальной части векторизуется) | **0.54 ms** |

Разбивка до переписывания: seed 0.09 ms, каждый из двух проходов 0.50 ms при
55 800 сэмплах — 4.5 ns на сэмпл-канал, то есть решатель упирался не в память,
а в латентность цепочки «умножить и взять максимум» вдоль строки. Проход разбит
на две части: то, что приходит из соседней (уже решённой) строки, независимо от
сэмпла к сэмплу и векторизуется; вдоль строки остаётся затухающий running
maximum — одно умножение и одно сравнение на сэмпл. Два канала идут по этой
цепочке рядом, и процессор исполняет их внахлёст: две цепочки стоят чуть
дороже одной.

`-O2` в GCC векторизует только циклы без эпилога (`very-cheap`), а у решателя
таких нет; `-fvect-cost-model=cheap` добавлен в release/profile флаги. На
симуляции (`make bench`) он не изменил ни счётчики, ни wall-clock.

`make bench`: `lighting moving_light` 1.229 → **0.551 ms**. Smoke на GPU:
`light_avg` (solve + upload) 2.55 → 1.82 ms — в полёте окно шире (viewScale
1.65) и бурение перестраивает световые блоки.

## Замер 2026-09-20: небо выше и бесплатно (EF-WLD-009)

Высота мира 1440 → 2048. В отличие от прошлого подъёма, грунт не тронут: земля
раскладывается в полосе `WORLD_GROUND_ROWS` (1440) у дна мира, а весь прирост
уходит в небо. Подъём от поверхности до линии космоса вырос с ~590 до ~1190
клеток, над линией космоса — 184 клетки вместо 130.

Массив клеток больше не инициализируется (у пустой клетки нет температуры), а
повторная генерация берёт свежий `calloc` вместо `memset`: страницы неба ядро не
выделяет, пока в них ничего не записано.

`make bench`, 16384×2048, та же машина:

| | 16384×1440, до | 16384×2048, после |
|---|---:|---:|
| `cells` (виртуально) | 270 MiB | 384 MiB |
| **RSS после генерации** | **272 MiB** | **162 MiB** |
| `init` | 60 ms | 0.2 ms |
| `generate` | 465 ms | 375 ms |
| `lighting moving_light` | 0.55 ms | 0.76 ms |
| `chaotic mixed` cells/tick | 119 455 | 119 005 |

Счётчики сценариев совпали с точностью до процента: сценарии bench-а заданы в
координатах полосы земли и сдвигаются на `WorldSkyRows` (`BenchY`). Одно
изменение содержательное: тела в сценариях `terrain *` раньше лежали на y=120 —
выше линии космоса мира в 1440, то есть в невесомости, и делали по одному
подшагу; теперь они там, где им положено, под тяжестью, и делают по десять.
Подорожал только solve света — поле выросло с 180 до 256 строк.

## Замер 2026-09-20: реакция вода/лава ищет только там, где есть что найти (EF-PERF-005)

Пункт 4 из «оставшегося долга»: `WorldTryMaterialReaction` сканировал восемь
соседей у каждой клетки воды и лавы каждый tick и почти всегда ничего не
находил. Эксперимент с выключенным сканом дал верхнюю границу выигрыша: `large
water` 2.95 → 1.95 ms.

Мир теперь держит по chunk-ам счётчики клеток воды и лавы, точные — их двигают
только три функции, которые пишут материал, — и клетка сканирует соседей лишь
когда её chunk или chunk за границей, на которой она стоит, содержит второй
материал. `make bench`, 16384×2048, та же машина, структурные счётчики
совпали до единицы:

| Scenario | до | после |
|---|---:|---:|
| large water | 2.95 ms | **2.30 ms** (−22%) |
| fire and lava | 0.92 ms | **0.77 ms** (−17%) |
| cryo ability | 2.47 ms | **2.04 ms** (−17%) |
| chaotic mixed | 4.20 ms | **3.75 ms** (−11%) |

Память: два массива `uint16_t` на chunk, 128 KiB на боевом мире. Регрессии:
`test_chunk_material_counts_match_a_brute_force_recount`,
`test_water_and_lava_react_across_a_chunk_border`.

## Замер 2026-09-20: тела сталкиваются друг с другом (EF-DYN-012)

До этого замера тела сталкивались только с миром, и куча обломков была
кусками в одних и тех же клетках. Парная фаза (`terrain_body_collision.c`) и
общий решатель контактов (`terrain_contact.c`, PGS по накопленным импульсам с
warm start) описаны в `docs/dynamic-terrain.md`; здесь — цена. `make bench`,
16384×2048, те же сценарии `terrain *`, плюс два новых: «64 pile» — восемь
колонн по восемь тел, каждое падает на предыдущее.

| Сценарий | до | после | пар/tick |
|---|---:|---:|---:|
| 1 awake | 0.003 ms | 0.010 ms | 0 |
| 16 awake | 0.054 ms | 0.072 ms | 0 |
| 64 awake в ряд | 0.221 ms | 0.290 ms | 0 |
| 64 sleeping | 0.001 ms | 0.006 ms | 0 |
| 64 shipped budget (40 awake) | 0.137 ms | 0.192 ms | 0 |
| cells at budget (64×64×32) | 0.825 ms | 0.962 ms | 0 |
| cells asleep | 0.001 ms | 0.006 ms | 0 |
| 64 pile | — | 0.444 ms | 560 |
| 64 pile shipped budget | — | 0.257 ms | 320 |

Три вещи стоили заметно, и все три сняты до замера выше: AABB тел пересчитывались
на каждую из 2016 пар (теперь один раз за подшаг); рабочее пространство
переписывало все 64 набора контактов каждый подшаг (теперь только непустые);
и тела, рождённые с флагом `wokeRecently`, будили соседей уже после того, как
сами уснули (флаг снимается при засыпании) — так «cells asleep» стоил 3.2 мс и
держал все 64 тела бодрствующими. Спящие строки остались на 0.006 мс: пары, в
которых спят оба, не рассматриваются.

Клеточные сценарии не изменились ни на один счётчик: столкновения мир не
трогают. Регрессии: `test_two_bodies_do_not_pass_through_each_other`,
`test_a_body_lands_on_another_and_the_pile_sleeps`,
`test_a_tall_pile_settles_and_sleeps`,
`test_a_heavy_body_moves_a_light_one_more_than_the_reverse`,
`test_an_off_centre_blow_spins_the_struck_body`,
`test_a_fast_body_does_not_tunnel_through_a_thin_one`,
`test_waking_a_body_wakes_what_rests_on_it`,
`test_pair_collision_is_deterministic`,
`test_ground_destroyed_under_a_sleeping_body_wakes_it`.

## Замер 2026-09-20: давление и импульс в жидкости (EF-WLD-010)

Модель — `docs/world-simulation.md` («Давление: сообщающиеся сосуды»,
«Импульс»). Клетка не выросла: напор живёт в `lifetime`, очередь импульсов —
1024 записи по 12 байт в `World`. `make bench`, 16384×2048, та же машина;
структурные счётчики клеточных сценариев без жидкости не изменились.

| Scenario | до | после | cells/tick до → после |
|---|---:|---:|---:|
| falling sand | 0.72 ms | 0.78 ms | 40 436 → 40 436 |
| large water | 1.94 ms | 2.02 ms | 67 470 → 67 470 |
| fire and lava | 0.64 ms | 0.95 ms | 32 011 → 37 415 |
| force ability | 0.75 ms | 0.84 ms | 29 445 → 29 536 |
| cryo ability | 1.72 ms | 1.86 ms | 67 401 → 66 918 |
| chaotic mixed | 3.10 ms | 3.46 ms | 120 064 → 122 754 |

Вода: плюс четыре процента — обновление напора у каждой клетки под
поверхностью и проход столба у каждой поверхностной. Лава в «fire and lava»
подорожала на половину: сценарий — блок лавы 190×60, вылитый на грунт, который
горит под ним, и у такого блока напор перестраивается на каждом движении, а
подъёмы (5 900 за 600 ticks) выносят лаву из глубины наверх, как и положено
вылитой куче. Это цена сцены, где всё движется; осевшее озеро стоит ноль.

Проба облёта всей карты (та же, что в EF-PERF-001, 3600 ticks покоя после
активации всего мира): 1408 chunks сразу после облёта, 168 через 60 ticks,
7 через 1200 — волны напора идут по озёрам со скоростью клетка за tick, —
**0 через 2400** и 0 через 3600. Трижды по дороге мир не засыпал, и каждая
причина — в комментариях кода: перенесённый с клеткой напор, ходьба клетки под
породой по карману воздуха, и два правила напора, которые расходились на
единицу у пруда с бугром.

Регрессии: `test_water_finds_one_level_through_a_channel`,
`test_draining_one_arm_lowers_the_other`,
`test_a_pipe_fills_its_hole_from_the_side`,
`test_a_pushed_pool_moves_and_settles_again`,
`test_a_push_travels_through_liquid`,
`test_fluid_impulses_are_bounded_and_refusals_counted`,
`test_a_sleeping_pool_wakes_for_a_change_and_sleeps_again`.

## Замер 2026-09-21: удар под океаном (EF-WLD-011)

Жалоба: силовой удар под поверхностью воды в океане — 60 fps падают до 15.
Headless-проба: production world, окно активации на океане, один
`WorldApplyForceBlast` на двадцать клеток ниже поверхности, 300 ticks:

| | avg ms/tick | worst tick | chunks в худший tick |
|---|---:|---:|---:|
| до модели давления (bedfc52) | 0.31 | 1.84 | 40 |
| первая модель давления (d5fea0b) | 3.41 | 9.12 | 172 |
| после | **0.20** | **0.53** | ~30 |

Причина: цепочка напоров хранилась в каждой клетке под поверхностью и
принимала боковые предложения, и любое изменение поверхности — а удар поднял
её на десятки клеток — уходило волной по столбам глубиной в сотню клеток,
будя chunk за chunk-ом, четыреста столбов в каждую сторону, пока потеря на
пути не съедала его. Теперь напор хранит только клетка с породой над собой, а
клетка под жидкостью читается подъёмом по столбу (`WORLD_LIQUID_CHAIN_REACH`);
поверхность спрашивает, что на неё давит, у двух соседних поверхностей и у
соседей под породой вдоль столба. Удар стоит дешевле, чем до модели давления,
потому что вода, разлетевшаяся от удара, теперь имеет импульс и падает
кучно, а не размазывается переносом.

Заодно: очередь импульсов выросла до 4096 (корона от тела размером с дом
толкает больше тысячи клеток), сопротивление персонажу снято, бур обращает
воду в пар. Регрессии: `test_water_never_slows_the_player`,
`test_the_drill_turns_water_to_steam`, `test_diving_in_fast_throws_a_crown`,
`test_a_body_hitting_water_splashes_and_slows`.

## Замер 2026-09-21: стена воды под пролётом и 256 тел (EF-WLD-012)

Жалоба: пролёт на сверхзвуке над озером почти не читается, а прибавить
надо не эффект, а число поднимающихся клеток. Теперь под пролётом вверх
бросается полоса воды до 14 рядов глубиной и 61 клетки шириной, импульсы
идут до четырёх клеток за тик (`WorldPushLiquidFast`, поле `pace` в тех же
12 байтах), поверхность под сверхзвуком вскипает. Headless-проба: полоса
океана 1024×80, персонаж на высоте двух клеток, 120 тиков:

| скорость | flyovers | клеток толкнуто | живых импульсов | высота столба | avg ms/tick | worst |
|---:|---:|---:|---:|---:|---:|---:|
| 118 (maxSpeed) | 74 | 4 275 | 67 | 12 | — | — |
| 300 | 76 | 27 531 | 144 | 43 | 1.22 | 2.76 |
| 380 (boost) | 67 | 39 972 | 457 | 48 | 1.94 | 3.72 |

Очередь импульсов — 32 768 (384 KiB), отказов нет. Вода, поднятая в воздух
вокруг персонажа, читается как брызги (`sprayTimer`), а не как вход в озеро.

`MAX_TERRAIN_BODIES` 64 → 256, `maxAwakeBodies` = все слоты,
`MAX_TERRAIN_DYNAMIC_CELLS` 786 432, `TERRAIN_PAIR_MAX_MANIFOLDS` 768. Арена
растров 33.75 MiB; `final_peak_rss` бенчмарка 330 MiB. Сценарии бенчмарка
теперь заполняют все слоты:

| сценарий | avg ms | p95 |
|---|---:|---:|
| all awake (256 тел, 49 152 клетки) | 2.12 | 3.80 |
| all sleeping | 0.050 | 0.062 |
| cells at budget (236 тел, 483 328 клеток) | 5.47 | 9.20 |
| cells asleep | 0.042 | 0.047 |
| all pile (1120 пар/тик) | 2.49 | 4.30 |

Спящие тела по-прежнему ничего не стоят; двести тел в воздухе одновременно —
редкость, и её цена ограничена клеточным бюджетом.

Заодно: обвал роняет кровлю плитами (трещины у опор + detach), песчинки не
держат компоненту, сироты по углу уходят с ней, отказ раскола помечает тело
и повторяется, приваривание под водой поднимает воду вместо того, чтобы
растворить тело. Регрессии: `test_a_failed_roof_falls_as_a_slab`,
`test_loose_material_never_anchors_a_component`,
`test_a_corner_orphan_goes_with_the_component`,
`test_a_body_welded_under_water_keeps_its_cells_and_the_water`.

## Замер 2026-09-25: мир 4096 в высоту, вход в атмосферу (EF-WLD-013)

Мир поднят с 2048 до 4096 строк, линии космоса и облаков — до 0.14/0.42
высоты: 573 строки открытого космоса и 1147 строк атмосферы, в которой идёт
вход. Бенчмарк поднят вслед (`BENCH_WORLD_HEIGHT` 4096): мерить мир, который
игра больше не поставляет, — мерить ничто.

| | 16384×2048 | 16384×4096 |
|---|---:|---:|
| cells, виртуально | 384 MiB | 768 MiB |
| rss после генерации | 136 MiB | 136 MiB |
| поле света | 8 MiB | 16 MiB |
| chunks | 32 768 | 65 536 |
| lighting still | 0.026 ms | 0.053 ms |
| lighting moving_light, до пропуска неба | 0.757 ms | 1.634 ms |
| lighting moving_light, после | — | **1.146 ms** |

Симуляция не изменилась ни в одном сценарии (спящие chunks не стоят ничего).
Свет — единственное, что росло с высотой: решение шло двумя проходами по
всем 512 строкам окна, а над землёй там пустой воздух. Теперь прямой проход
начинается с первой строки, где есть непрозрачность, свечение или лампа, а
обратный останавливается, как только строка свечения в открытом воздухе вся
ниже 0.5/255 — полшага 8-битной текстуры, в которую поле загружается, то есть
то, что в кадре и так ноль (`World.lightStats.skippedRows`). Остаток сверх
прежних 0.76 мс — засев всех строк и подъём свечения лампы в небо: в
открытом воздухе оно гаснет на 3 % за строку и остаётся видимым ~200 строк.

Вход в атмосферу: одна проходка по живым телам на фиксированном шаге и
несколько клеток опаливания вокруг горящего персонажа; купол рисуется
блоками, размер блока растёт с телом, так что его стоимость — число блоков
(сотни), а не площадь. Ниже 200 клеток/с ничего из этого не работает.
Регрессии: `test_the_atmosphere_is_a_corridor_between_the_lines`,
`test_falling_through_the_air_burns_the_player_without_slowing_them`,
`test_cruising_through_the_air_never_burns`,
`test_a_body_falling_from_space_is_slowed_and_heated`,
`test_the_light_solve_skips_the_empty_sky`.

## Замер 2026-09-25: палитры, оттенок клетки, солнце и луна (EF-RND-004)

`Cell` остался 12 байт: оттенок (`shade`, 6 бит) живёт рядом с `heatHeld`
(2 бита) в бывшем байте выравнивания. Растры тел получили третью плоскость,
`shade`, 1 байт на клетку: 256 × 27 648 = 6.75 MiB, выделяется один раз.
Сборщик страниц читает для каждой клетки ещё соседей сверху и снизу — строки
уже в кэше — и счётчик жидкости по столбцу; value-noise считается только при
перестройке грязного чанка, а не каждый кадр. Солнце и луна — несколько
десятков прямоугольников в кадр в экранном пространстве.
Регрессия: `test_a_cell_keeps_its_shade_through_moves_and_bodies`.

## Замер 2026-09-25: мир замкнут по горизонтали (EF-WLD-014)

Правый край мира соединён с левым. Колонка берётся по модулю ширины в
`WorldIndex`, на который опирается каждый доступ к клетке, — в самом горячем
месте игры. Первая версия (`x >= 0 && x < width`, затем `%`) стоила ~10 % на
сценариях с массой движущихся клеток; быстрый путь из одного беззнакового
сравнения и одного шага на ширину без деления — около 4 %:

| сценарий | до | первая версия | итог |
|---|---:|---:|---:|
| falling sand | 0.903 | 1.012 | 0.942 |
| chaotic mixed | 3.888 | 4.363 | 4.023 |

Счётчики работы (cells/tick, chunks/tick) не изменились ни в одном сценарии.
Решение света через шов копирует окно (≈ 336 × 512 × 2 плоскости) в свои
плоскости и обратно только тогда, когда окно шов пересекает.

## Замер 2026-09-25: логичная и фэнтезийная генерация (EF-WLD-015)

Горы, снег, базальт, каверны, тоннели, линзы, руины, подземелья, шахты,
склепы, острова. Генерация 16384×4096:

| | время |
|---|---:|
| до (одна высота 4096) | ~600 ms |
| с новыми проходами, поиск верха столбца с y = 0 | 1370–6000 ms |
| поиск верха столбца от верха полосы земли | ~600 ms |

Основа (`GenerateBaseTerrain`) — 340–750 ms в зависимости от прогрева
страниц; каверны, тоннели, строения и острова вместе — десятки миллисекунд.
Запечатывание подземной жидкости смотрит только чанки с ненулевыми
счётчиками воды/лавы и кэширует верх каждого столбца.

Облёт всей карты окном активации 960×576 (EF-PERF-001) с новыми материалами
(сыпучий снег, линзы, жаровни, подземные озёра):

| | active chunks | cells/tick |
|---|---:|---:|
| сразу после облёта | 340 | — |
| через 60 ticks | 262 | 269 312 |
| через 600 ticks | 54 | 55 296 |
| через 3600 ticks | **0** | **0** |

### Сверка обновления EF-WLD-013…015 целиком

После всех пяти фаз (зум, меню, замкнутый мир, генерация, задний план)
счётчики работы всех сценариев `make bench` совпадают с предыдущими. Время на
этой машине в тот день плавало в полтора-два раза (батарея, троттлинг): один и
тот же бинарник коммита 3c7f855 показал `traversal` 3.5 ms и 1.7 ms с разницей
в минуты. Поэтому сравнение сделано поочерёдными прогонами двух бинарников:

| traversal | avg | p50 | p95 |
|---|---:|---:|---:|
| 3c7f855 (до генерации) | 1.69 / 1.80 | 0.20 / 0.23 | 5.26 / 5.50 |
| итог | 1.35 / 1.43 | 0.21 / 0.18 | 4.10 / 4.55 |

Мерить время только так — два бинарника подряд, — когда машина не в покое.
