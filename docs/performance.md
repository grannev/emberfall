...
## EF-RND-003 — рацион на перестроение чанков
...
на каждую. Рацион в 128 чанков — это две полные страницы, поэтому приходящая
страница никогда не ждёт, а ограничение срабатывает только на всплеске
(изменение размера окна, регенерация), размазывая работу на кадр-другой вместо
того, чтобы уронить один.

Ограничение стоит **до** работы, а не после: визитор загрузки вызывается уже с
готовым блоком пикселей, и отказ от него экономил бы только `UpdateTextureRec`.
Первая попытка ограничивала именно визитор — отложенные чанки оставались
грязными и перестраивались каждый кадр заново, `prepare` вырос до 68 мс.

**Что было измерено и отвергнуто.** Захват страниц на кадр вперёд — очевидный
напарник рациона — оказался в несколько раз *хуже*: решатель света и пометка
перевысвеченных чанков идут по тому же прямоугольнику, и его расширение стоило
намного больше, чем выигрывала ранняя загрузка (11–15 мс на кадр при нуле новых
страниц). Уменьшение страницы со 256 до 128 тоже не окупилось: всплеск при
въезде вчетверо меньше, но постоянных страниц вчетверо больше, и суммарно вышло
хуже. Обе ветки откачены; в коде остались комментарии о том, что их пробовали, —
чтобы следующий, кому это придёт в голову, не мерил заново.
