...
`TERRAIN_INTERACTION_MAX_SUBSTEPS`.

Захват удерживается на `F`; берётся ближайшее к прицелу тело в пределах
`grabDistance` и от игрока, и от прицела. Точка захвата притягивается к
занятой клетке и хранится в координатах растра тела; если эта клетка исчезла,
захват следует за ней на отколовшийся кусок или чисто заканчивается. Удержание — пружина с демпфированием, а не присвоение позиции.
...
`DynamicTerrainRaycast` (в `dynamic_terrain.h`) отвечает, какое тело первым
встаёт на пути отрезка. Марш идёт в системе координат каждого кандидата, шагом в
долю клетки: отрезок способен задеть угол повёрнутой клетки на очень короткой
хорде.
